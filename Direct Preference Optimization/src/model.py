...
DPO fine-tunes a language model directly on preferences, so this is the only
network in the method — there is no separate reward model. The same `TinyLM`
serves as both the frozen reference policy π_ref and the trainable policy π_θ.
It is deliberately minimal: a couple of causal self-attention blocks over a
tiny vocabulary, enough to learn the toy "sort these tokens" task on CPU.
"""
...
class TinyLM(nn.Module):
    ...
    def sequence_logprob(
        ...
        response. Returns (b,) = log π(response | prompt), the quantity DPO needs.
        """
        logits = self(full[:, :-1])

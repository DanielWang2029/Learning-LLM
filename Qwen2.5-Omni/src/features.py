"""From-scratch log-Mel spectrogram front-end (Qwen2.5-Omni §2.1).

Qwen2.5-Omni resamples audio to 16 kHz and transforms the waveform into a
128-channel Mel spectrogram with a 25 ms window and a 10 ms hop. We implement
the whole thing with numpy only — no torchaudio, no librosa:

    waveform -> framing (Hann, 25 ms) -> rFFT -> power -> Mel filterbank -> log

At 16 kHz a 10 ms hop gives 100 Mel frames/second; the encoder's conv stem then
downsamples by 4, so each encoder frame corresponds to ~40 ms of audio (the
paper's "one temporal ID per 40 ms").
"""
...
N_MELS = 128         # 128-channel Mel spectrogram
SAMPLE_RATE = 16_000
